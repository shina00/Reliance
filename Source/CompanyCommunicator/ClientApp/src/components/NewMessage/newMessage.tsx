...
class NewMessage extends React.Component<INewMessageProps, formState> {
        ...
        this.getTeamList().then(() => {
            if ('id' in params) {
                ...
                this.setState({
                    ...
                    if (this.state.btnlink2) {
                        let link = this.state.btnlink2;
                        adaptiveCard.onExecuteAction = function (action) { window.open(link, '_blank'); };
                    }
                })
            }
    ...
    private getItem = async (id: number) => {
        try {
            ...
            this.setState({
                ...
                btnlink2: draftMessageDetail.buttonLink,
            ...
            if (this.state.page === "CardCreation") {
                return (
                    <div className="taskModule"  >
                        <Flex column className="formContainer" vAlign="stretch" gap="gap.small" styles={{ background: "white",border:"solid 2px grey" }}>
    ...
    private onBtnTitleChanged = (event: any) => {
        ...
        if (event.target.value && this.state.btnlink2) {
            setCardBtn(this.card, event.target.value, this.state.btnlink2);
            this.setState({
    ...
    private onBtnTitleChanged2 = (event: any) => {
        const showDefaultCard = (!this.state.title && !this.state.imageLink && !this.state.summary && !this.state.author && !event.target.value && !this.state.btnlink2);
        setCardTitle(this.card, this.state.title);
        setCardImageLink(this.card, this.state.imageLink);
        setCardSummary(this.card, this.state.summary);
        setCardAuthor(this.card, this.state.author);
    ...
    private updateCard = () => {
        ...
        const link2 = this.state.btnlink2;
        adaptiveCard.onExecuteAction = function (action) { window.open(link,link2, '_blank'); 
    } 
    }
}

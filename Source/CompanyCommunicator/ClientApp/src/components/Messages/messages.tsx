...
import title from '../image/title.png'; // with import
import recipient from '../image/recipi.png'; // with import
import sent from '../image/sent.png'; // with import
...
class Messages extends React.Component<IMessageProps, IMessageState> {
    ...
    public render(): JSX.Element {
        ...
        const processItem = (message: any) => {
            ...
            const out = {
                ...
                styles: { margin: '0.2rem 0.2rem 0 0',color: "	#A9A9A9" },
            };
            return out;
        };
        ...
        if (this.state.loader) {
            ...
            return (<div className="results" style={{backgroundColor:"	#A9A9A9"}}>{this.localize("EmptySentMessages")}</div>);
        }
        else {
            return (
                <List selectable items={allMessages} className="list" style={{backgroundColor:"	#A9A9A9",fontFamily:"sans,georgia"}}/>
            );
        }
    }
    ...
    private processLabels = () => {
        const out = [{
            key: "labels",
            content: (
                <Flex vAlign="center" fill gap="gap.small" style={{backgroundColor:"	#A9A9A9"}}>
                     <img src={title} /> 
